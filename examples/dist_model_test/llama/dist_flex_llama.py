...
from common_utils import get_filename
...
from llama_config import get_llama_config
...
from data_types import GB, str2bool, array_1d, array_2d, array_3d, array_4d
    ...
    def __init__(self, config, env, path, policy, pipeline_rank,
        ...
        for i in range(layer_start_ids[pipeline_rank], layer_start_ids[pipeline_rank + 1]):
            if self.policy.sep_layer:
                layers.append(LlamaAttention(self.config, self.env, self.policy, i))
                layers.append(LlamaMLP(self.config, self.env, self.policy, i))
            else:
                layers.append(LlamaDecoderLayer(self.config, self.env, self.policy, i))
...
def run_flexgen_dist(args):
    ...
    model = DistLlamaLM(llama_config, env, args.path, policy, args.rank,
    ...
    log_str = (f"model size: {llama_config.model_bytes()/GB:.3f} GB\t"
               f"cache size: {cache_size/GB:.3f} GB\t"
               f"hidden size (prefill): {hidden_size/GB:.3f} GB\n"
